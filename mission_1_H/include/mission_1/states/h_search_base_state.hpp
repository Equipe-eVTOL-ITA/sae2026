...
public:
    ...
    std::string act(fsm::Blackboard &blackboard) override {
        ...
        case Phase::MOVE_TO_SIDE1:
            if (nav_mover_.update(drone_, side1_pos_, 0.5f, 0.5f, 0.20f)) {
        ...
        case Phase::RETURN_HOME:
            if (nav_mover_.update(drone_, home_pos_, 0.5f, 0.5f, 0.20f)) {
        ...
        case Phase::MOVE_TO_SIDE2:
            if (nav_mover_.update(drone_, side2_pos_, 0.5f, 0.5f, 0.20f)) {
...
private:
    ...
    bool search_step(const std::array<Eigen::Vector3d, 4>& waypoints) {
        ...
        bool at_wp = wp_mover_.update(drone_, target, 0.4f, 0.5f, 0.20f);
        if (at_wp) {
    ...
    TrapezoidalMover nav_mover_;  // fases MOVE_TO_SIDE1, RETURN_HOME, MOVE_TO_SIDE2
    TrapezoidalMover wp_mover_;   // waypoints de dwell em SEARCH_SIDE1/2
};
